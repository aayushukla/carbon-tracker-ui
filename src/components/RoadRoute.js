...
function RoadComponent(props) {
  ...
  useEffect(() => {
    ...
    async function getSeaData() {
      ...
      let maxCO2 = 0;
      let maxSN = 0;
      const data = await GroundTransportService.getGroundTransportData();
      const co2 = data;
      let count = co2.length;
      rows = await GroundTransportService.getGTRowsDataForDashBoard();
      columns = await GroundTransportService.getGTColumnDataForDashBoard();
                ...
                <main style={{ margin: '2%' }}>
                    ...
                    <Row xs={1} md={2} className="g-4">
                        ...
                        <Col>
                            <Card style={{ alignItems: 'center', display: 'flex', justifyItems: 'center' }}>
                                <Card.Header style={{ fontSize: '20px', textAlign: 'center', color: 'white', width: "100%", backgroundImage: "linear-gradient(130deg,#6304ff,#23adf3)" }}>
                                    Total CO2 Emission - Ground Transportation
                                </Card.Header>
                                <CircularProgressbar
                                styles={{
                            ...
                            <Card  style={{ marginTop: '15px' }}>
                                <Card.Header style={{ fontSize: '20px', textAlign: 'center', color: 'white', width: "100%", backgroundImage: "linear-gradient(130deg,#6304ff,#23adf3)" }}>
                                    Trends in Ground Transportation
                                </Card.Header>
                                <Card.Body>
                        ...
                        <Card className='text-center'>
                                <Card.Body >
                                    Are you looking for Historical Data of Ground Transportation?
                                    <br></br>
